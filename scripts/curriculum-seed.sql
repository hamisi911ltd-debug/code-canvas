...
By the end of this course you''ll run that loop for real, building a small AI-powered app from a blank file to something that runs in your browser.', 1, 15);
...
You don''t need to memorize all of this today. You need to recognize it when an AI assistant generates it, so you can tell whether the code it wrote actually does what you asked for.', 2, 15);
...
When you ask an AI to "call this API," it''s writing exactly this pattern under the hood. Knowing it helps you spot when the error handling is missing — a very common AI-generated bug.', 3, 15);
...
This is the core building block every AI feature in this course is built on.', 4, 15);
...
Being specific isn''t a crutch — it''s the actual skill. The best vibe coders are precise communicators first.', 5, 15);
...
You don''t need to know every utility type today — you need to recognize the shape so you can look the rest up when you need them.', 1, 15);
...
**Composing components** — building bigger UI from smaller pieces — is the core React skill. A page is just components, inside components, inside components.', 2, 15);
...
Getting the dependency array right is the single most common source of React bugs — get comfortable with it early.', 3, 15);
...
You don''t need 100% coverage. A handful of well-chosen tests around your riskiest logic beats hundreds of tests that just restate what the code already says.', 4, 15);
...
This is an optional deep-dive — the core skills from earlier lessons (components, state, props) transfer directly. What''s different here is the rendering target and the styling system.', 5, 15);
...
**Streams** let Node process data (a big file, a long response) in chunks instead of loading the whole thing into memory at once — important once you''re past toy examples.', 1, 15);
...
Getting these right matters — the client code (and whoever debugs it later) relies on them to know what actually happened.', 2, 15);
...
**Relationships** matter as much as individual tables — a course belongs to a category, a lesson belongs to a course. Deciding those foreign keys up front saves you from painful migrations later.', 3, 15);
...
D1 is genuinely SQL — if you know SQLite, you already know D1. The only new thing is the binding API for talking to it from a Worker.', 4, 15);
...
**CSRF** is an attack where a malicious site tricks a logged-in user''s browser into making a request to your API. `sameSite` cookies are your main defense against it in most modern apps.', 5, 15);
INSERT OR IGNORE INTO courses (id, category_id, title, slug, description, level, duration_minutes, token_cost, published) VALUES ('course-ai-integration', 'cat-ai-ml', 'Integrating AI into Real Products', 'integrating-ai-real-products', 'Move beyond toy demos. Wire large language models into production apps: streaming, structured output, retrieval, and the patterns that keep it reliable.', 'intermediate', 90, 1, 1);
...
**System prompts are your main lever for behavior** — "You are a strict JSON-only API, never include prose" changes the model''s output far more reliably than asking nicely in the user message.', 2, 15);
...
Some APIs offer a dedicated "JSON mode" that guarantees valid JSON — prefer that over prompting alone when it''s available.', 3, 15);
...
**Why this matters:** it lets the model answer accurately about *your* courses, *your* docs, *your* product — things it was never trained on — and it dramatically reduces made-up answers, because the model is grounding its response in text you actually gave it rather than guessing from memory.', 4, 15);
...
Treat the model like an unreliable but talented contractor: give it clear instructions, check its work, and don''t hand it the keys to anything irreversible without a check in between.', 5, 15);
...
**Evaluation isn''t optional for anything that ships.** Even a rough scoring rubric you check by hand against 20 real examples will catch problems that "it looked fine when I tried it twice" never will.', 6, 15);
...
- Most real apps mix all three per-route, which is exactly what modern frameworks like TanStack Start let you do — choose per page, not per app.', 1, 15);
...
This is what removes the need for a totally separate backend service for most apps — your "backend" is just the server functions living next to your routes, deployed as one unit.', 2, 15);
...
- Deploy from a single branch (usually `main`) so there''s one source of truth for what''s live', 3, 15);
...
**wrangler.jsonc** is where you declare what your Worker has access to — D1 databases, KV namespaces, environment variables — so the binding is explicit and version-controlled, not configured by clicking around a dashboard.', 4, 15);
...
**Why not just put everything in D1?** Databases are built for structured, queryable rows — not for storing large binary blobs efficiently. Splitting structured data (D1) from large files (R2) is the standard pattern, not a Cloudflare-specific quirk — it''s true of SQL databases in general.', 5, 15);
...
**Running E2E tests in CI** means they run headless against a freshly built version of the app on every push — the exact regression check that catches "I refactored the auth form and silently broke sign-up" before it reaches anyone real.', 6, 15);
INSERT OR IGNORE INTO courses (id, category_id, title, slug, description, level, duration_minutes, token_cost, published) VALUES ('course-ui-design-figma', 'cat-design', 'Design Systems in Figma', 'design-systems-figma', 'Tokens, components, layouts, and developer handoff. Design clean, professional screens before you build them.', 'beginner', 60, 1, 1);
...
**Why this matters for handoff:** when a developer asks "what color is this," the answer should be a token name they can find in code (`text-primary`), not "whatever hex value looked right in this one spot."', 1, 15);
...
Designing in components from day one is what makes a "design system" a system, rather than just a folder of mockups.', 2, 15);
