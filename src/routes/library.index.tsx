      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-12">
        {isLoading ? (
          <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="h-40 sm:h-64 rounded-2xl bg-muted/40 animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((mod: any) => {
              ...
              return (
                <Link
                  ...
                  className="group relative overflow-hidden rounded-xl sm:rounded-2xl border border-border bg-card p-3 sm:p-6 hover:border-primary/50 transition-all card-glass flex flex-col"
                >
                  <div className="flex items-start justify-between mb-3 sm:mb-5">
                    <div className="grid h-9 w-9 sm:h-12 sm:w-12 place-items-center rounded-lg sm:rounded-xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition">
                      <Icon className="h-4 w-4 sm:h-5 sm:w-5" />
                    </div>
                    {total > 0 && <Badge variant="secondary" className="text-[10px] sm:text-xs px-1.5 py-0.5">{total}</Badge>}
                  </div>
                  <h3 className="font-display text-sm sm:text-xl font-bold group-hover:text-primary transition line-clamp-2">{mod.name}</h3>
                  {mod.description && <p className="mt-1 text-[11px] sm:text-sm text-muted-foreground flex-1 line-clamp-2 hidden sm:block">{mod.description}</p>}
                  <div className="mt-2 sm:mt-5 flex flex-wrap gap-1.5 sm:gap-3 text-[10px] sm:text-xs text-muted-foreground">
                    {mod.videoCount > 0 && (
                      <span className="flex items-center gap-1"><Play className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" /> {mod.videoCount}</span>
                    )}
                    {mod.notesCount > 0 && (
                      <span className="flex items-center gap-1"><FileText className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" /> {mod.notesCount}</span>
                    )}
                    {mod.resourceCount > 0 && (
                      <span className="flex items-center gap-1"><Link2 className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" /> {mod.resourceCount}</span>
                    )}
                  </div>
                  <div className="mt-2 sm:mt-4 hidden sm:flex items-center gap-1 text-sm text-primary opacity-0 group-hover:opacity-100 transition">
