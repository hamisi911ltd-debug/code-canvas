...
import { BookOpen, CheckCircle2, Flame, ArrowRight } from 'lucide-react'
      ...
      <section className="bg-glow relative">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 pt-16 pb-10">
          ...
          <h1 className="font-display text-4xl sm:text-5xl font-bold mt-2">
            Welcome back, <span className="text-gradient">{user?.display_name ?? 'builder'}</span>.
          </h1>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {(rows ?? []).map((r) => (
              <Link
                ...
                className="group rounded-2xl border border-border bg-card p-5 hover:border-primary/50 transition card-glass"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-xs text-primary uppercase tracking-wider capitalize">{(r.courses as any).level}</div>
                    <h3 className="mt-1 font-display text-lg font-semibold group-hover:text-primary">{(r.courses as any).title}</h3>
                  </div>
