...
import { getCommunityPosts, createPost } from '@/server-functions/data'
...
function Community() {
  ...
  const create = useMutation({
    mutationFn: () => createPost({ data: { title, content } }),
    onSuccess: () => {
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 pt-16 pb-10 flex flex-wrap items-end justify-between gap-4">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button
                onClick={() => { if (!user) navigate({ to: '/auth' }) }}
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Button onClick={() => create.mutate()} disabled={create.isPending} className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
                  Post
                </Button>
              </div>
            </DialogContent>
