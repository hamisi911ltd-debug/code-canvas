      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-14">
        {isLoading ? (
          <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-44 sm:h-64 rounded-2xl bg-muted/40 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
            {(tracks ?? []).map((track: any, idx: number) => {
              ...
              return (
                <Link
                  ...
                  className="group relative flex flex-col overflow-hidden rounded-xl sm:rounded-2xl border border-border bg-card hover:border-primary/50 transition-all duration-300 card-glass"
                  style={{
                ...
                >
                  {/* Gradient top */}
                  <div className={`h-20 sm:h-28 bg-gradient-to-br ${gradient} flex items-center justify-center relative overflow-hidden`}>
                    <div className="absolute inset-0 grid-pattern opacity-20" />
                    <div className="relative grid h-10 w-10 sm:h-14 sm:w-14 place-items-center rounded-xl sm:rounded-2xl bg-background/60 backdrop-blur border border-border group-hover:scale-110 transition-transform duration-300">
                      <Icon className="h-5 w-5 sm:h-7 sm:w-7 text-primary" />
                    </div>
                    {certified && (
                      <div className="absolute top-2 right-2 flex items-center gap-1 rounded-full bg-primary px-2 py-0.5 text-[10px] sm:text-xs font-medium text-primary-foreground">
                  ...
                  <div className="flex flex-1 flex-col p-3 sm:p-6">
                    <h3 className="font-display text-sm sm:text-xl font-bold group-hover:text-primary transition-colors line-clamp-2">{track.name}</h3>
                    {track.description && (
                      <p className="mt-1 text-[11px] sm:text-sm text-muted-foreground leading-relaxed flex-1 line-clamp-2 hidden sm:block">{track.description}</p>
                    )}
                    <div className="mt-2 sm:mt-5 flex items-center justify-between">
                      <Badge variant="outline" className="border-border text-muted-foreground text-[10px] sm:text-xs capitalize px-1.5 py-0.5">
                        {track.slug === 'vibecoding' ? 'Popular' : track.slug === 'frontend' ? 'In-demand' : 'Track'}
                      </Badge>
                      <span className="hidden sm:flex items-center gap-1.5 text-sm text-primary opacity-0 group-hover:opacity-100 transition-opacity">
